...
def Read(cli,adr):
    try:
        while True:
            cli.send("请注册用户名".encode("utf8"))
            resu = cli.recv(1024)        # 接受昵称
            tem = resu.decode("utf8")   # 昵称解码
            num=0
            for v,r in users.items():
                if r["name"]==tem:
                    cli.send("该用户名已存在".encode("utf8"))
                    break
            users[adr]["name"]=tem
            break

        resu = cli.recv(1024)        # 接受聊天对象
